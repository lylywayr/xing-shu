...
import { autoExclusionReasons, capabilityVerificationComplete, evidenceGroups, governanceCounts } from './governance'
...
describe('governance traceability', () => {
  ...
  it('requires all three runtime protocol evidence records', () => {
    const verified = model({ capability_evidence: {
      structured_output: { supported: true, confidence: 1, source: 'runtime_probe', level: 'protocol', checked_at: '' },
      tools: { supported: false, confidence: 1, source: 'runtime_probe', level: 'protocol', checked_at: '' },
      vision: { supported: false, confidence: 1, source: 'runtime_probe', level: 'protocol', checked_at: '' },
    } })
    expect(capabilityVerificationComplete(verified)).toBe(true)
    expect(capabilityVerificationComplete(model())).toBe(false)
  })
