...
import { canValidateProvider, emptyProviderDraft, providerDraftMissing, providerToDraft, validationSteps, type ProviderDraft, type ProviderValidation } from '../services/provider-registry'
...
const missingFields = computed(() => providerDraftMissing(draft.value, editing.value))
const validateHint = computed(() => !props.credentialReady && !readOnly.value ? '加密主密钥未配置' : missingFields.value.length ? `还需填写：${missingFields.value.join('、')}` : '配置完整，可以验证')
const localhostHint = computed(() => /^https?:\/\/(127\.0\.0\.1|localhost)(:\d+)?(\/|$)/i.test(draft.value.base_url))
watch(() => [props.open, props.provider] as const, async ([open]) => {
  ...
  await nextTick(); body.value?.scrollTo({ top: 0 }); focusableElements(wizard.value as HTMLElement)[0]?.focus(); document.addEventListener('keydown', onKeydown)
}, { immediate: true })
...
function payload() { return { ...draft.value, id: editing.value ? draft.value.id : '', api_key: draft.value.api_key || undefined } }
...
<template>
  <div v-if="open" class="provider-wizard-layer" @click.self="emit('close')">
    <aside ref="wizard" class="provider-wizard" role="dialog" aria-modal="true" aria-labelledby="provider-wizard-title">
      ...
      <div ref="body" class="provider-wizard-body">
        ...
        <div class="wizard-form">
          <label><span>名称</span><input v-model.trim="draft.name" :disabled="readOnly" autocomplete="off" placeholder="例如 Trae"/><small v-if="editing">内部标识：{{ draft.id }}（由星枢维护）</small></label>
          <label class="wizard-wide"><span>OpenAI-compatible Base URL</span><input v-model.trim="draft.base_url" :disabled="readOnly" inputmode="url" autocomplete="url" placeholder="https://api.example.com 或 /v1"/><small>星枢会自动规范为 /v1，并验证 /models 与 /chat/completions。</small><small v-if="localhostHint" class="local-url-warning">Docker 中 127.0.0.1 指向星枢自身；NAS 宿主机服务请改用 http://host.docker.internal:端口/v1。</small></label>
