...
func TestKnowledgeDecisionRejectsInvalidTransition(t *testing.T) {
	...
	KnowledgeDecision(rt).ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/api/admin/knowledge/decision", strings.NewReader(`{"id":"k1","status":"trusted"}`)))
...
func TestKnowledgeRestoreRequiresExpiredOrRejected(t *testing.T) {
	...
	rt := &Runtime{Knowledge: store}
	w := httptest.NewRecorder()
	KnowledgeRestore(rt).ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/api/admin/knowledge/restore", strings.NewReader(`{"id":"k1","status":"validated"}`)))
	if w.Code != http.StatusConflict {
