...
func TestShadowBatchRequiresExplicitCandidateModel(t *testing.T) {
	...
	ShadowBatchStart(rt, manager, NewOps(), runtime.NewShadowBatches()).ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/api/admin/shadow/batches/start", strings.NewReader(`{"knowledge_id":"k1","review_ids":["r1"]}`)))
